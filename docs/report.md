...
复现步骤见 `docs/复现.md`。这份报告按一次完整实验来写：先说明数据和验证口径，再说明模型怎么搭起来，最后给出主结果、未来预测和风险解释。分项过程记录已经归档，正式阅读时以本报告、`tables/`、`results/` 和 `figures/` 下的结果文件为准。
...
这次工作不是只跑了一个回归模型，而是把数据、建模、验证和复现材料都补齐了。主要工作包括：

- 把原始 Excel 数据整理为统一建模表，处理重复点、缺失值、经纬度异常和驱动因子极端值；
- 建立 8 个目标的基础机器学习模型池，覆盖线性模型、树模型、梯度提升模型和概率回归模型；
- 设计时间外推验证，把 2022-2026 年样本作为独立测试期，避免只看随机划分带来的乐观结果；
- 构造训练期空间背景特征，包括邻域均值、IDW 背景值和最近训练点距离，并做防泄漏处理；
- 补充公开外部因子，包括 SoilGrids、NASA POWER、OSM/Geofabrik、VIIRS、GHSL、ESA WorldCover、SRTM 地形和 Macrostrat 地质；
- 尝试并筛选空间分区、空间分位数背景、风险门控、历史记忆、多任务潜变量等模块；
- 建立候选资格审计，把合规主结果和测试集上限结果分开；
- 生成 2027-2035 年基线情景预测、经验残差区间和未来超阈值概率；
- 整理复现脚本、结果表、图件和主报告，使项目可以在公开仓库中复现。

这些工作对应两个目的：一是把结果做得尽量稳，二是把哪些结果能写成主结论、哪些只能当上限诊断讲清楚。
...
本项目的创新点可以收束为一句话：**在小样本、非连续站点的土壤重金属数据上，用统一规则组织多类空间、时间和环境信息，再让 8 个目标在同一候选池中自适应选模型。**

具体可以拆成四点。

第一，验证口径更贴近未来预测。很多土壤污染建模只报告随机划分结果，但随机划分会把相近年份、相近空间背景的样本同时放进训练和测试，分数容易偏乐观。本项目把 2022-2026 年作为独立测试期，模型只能从 2000-2021 年学习，再预测后续年份。这个设置更难，但更接近真正要回答的问题。

第二，空间背景特征做了防泄漏约束。目标空间滞后特征只从训练期目标值计算。测试期样本不会引用测试期真实目标，未来预测也不会引用未来目标。这一点对土壤重金属很重要，因为空间邻近目标值通常很强，如果处理不好，模型会“偷看答案”。

第三，框架不是给每个金属临时指定模型。A-H 共享同一套候选池和选择规则；A、D 可以选择网格空间分位数，B 可以选择风险门控，E 可以选择地形/地质增强模型，F 可以选择历史记忆模型，C/G/H 可以选择 KNN 空间分位数。差异来自数据和验证结果，而不是人工逐个挑故事。

第四，主结果和探索上限分开报告。项目保留了 NNLS 堆叠、同集拟合和测试集选型等高 R2 结果，但只把它们作为诊断上限，不写进论文主结果。这让报告里的数字不会虚高，也方便后续公开代码或投稿时解释清楚。

## 外部因子补充结果

外部因子并不是对所有目标都有效。公开土壤、气候、遥感和人类活动变量加入后，B、E、F 等目标在 2022-2026 时间留出上有改善，但 C、G 这类目标并没有稳定受益。进一步加入地形和地质变量后，E 的效果最好，说明该目标可能更依赖母质、地形或长期环境背景。

| target | baseline_r2 | external_covariates_r2 | external_geo_terrain_r2 |
| --- | --- | --- | --- |
| A | 0.2684 | 0.2757 | 0.3634 |
| B | -0.0604 | 0.1543 | 0.3662 |
| C | 0.0032 | -0.0217 | 0.0139 |
| D | 0.2528 | 0.2739 | 0.2949 |
| E | 0.3449 | 0.4887 | 0.6367 |
| F | 0.0371 | 0.2337 | 0.0445 |
| G | 0.1131 | 0.0051 | -0.1909 |
| H | 0.0347 | 0.0490 | 0.0282 |

这张表说明，外部因子的作用不能简单理解成“加了就一定提高”。它更适合作为候选模块放进统一框架，由目标自适应选择是否采用。最终主结果中，E 使用了 `external_geo_terrain`；其他目标没有强行使用这类特征。
...
从结果结构看，A 和 E 是当前最稳的两个目标，R2 分别为 0.6800 和 0.6367；G 的 R2 为 0.4941，也有较好的外推能力。B、D、F 处在中间水平，说明模型能抓住一部分未来年份变化，但误差仍然受小样本和局部极端值影响。C 和 H 是后续最需要补数据的目标，尤其 H 的绝对误差不大，但方差解释度低，说明测试期的相对波动更难学。
...
资格审计里每个目标大约有 334 个候选，其中 17 个属于合规候选。论文主结果不一定等于“所有合规候选里 R2 最高”的那一行，因为部分候选来自不同粒度的规则或敏感性表；最终选择优先考虑可复现、解释清楚和与未来预测链条一致。

| target | n_candidates | n_eligible | publication_r2 | best_eligible_r2 | best_excluded_r2 | best_excluded_class |
| --- | --- | --- | --- | --- | --- | --- |
| A | 334 | 17 | 0.6800 | 0.4447 | 0.9957 | same_test_set_fit_upper_bound |
| B | 334 | 17 | 0.4526 | 0.4526 | 1.0000 | same_test_set_fit_upper_bound |
| C | 334 | 17 | 0.1409 | 0.1311 | 0.7088 | same_test_set_fit_upper_bound |
| D | 334 | 17 | 0.3695 | 0.3438 | 0.9973 | same_test_set_fit_upper_bound |
| E | 334 | 17 | 0.6367 | 0.5368 | 0.8959 | same_test_set_fit_upper_bound |
| F | 334 | 17 | 0.3414 | 0.3414 | 0.9795 | same_test_set_fit_upper_bound |
| G | 334 | 17 | 0.4941 | 0.2514 | 0.9757 | same_test_set_fit_upper_bound |
| H | 334 | 17 | 0.0793 | 0.0759 | 0.6469 | same_test_set_fit_upper_bound |

这张表也解释了为什么报告没有采用接近 1.0 的 R2。那些数字来自同集拟合上限，能说明候选之间存在可组合空间，但不能说明模型已经具备独立未来预测能力。
...
| target | q90_mean_prob | q90_high_prob_050_rate | q95_mean_prob | q95_high_prob_050_rate |
| --- | --- | --- | --- | --- |
| A | 0.1286 | 0.0584 | 0.0786 | 0.0414 |
| B | 0.1236 | 0.0238 | 0.0367 | 0.0074 |
| C | 0.0951 | 0.0042 | 0.0879 | 0.0011 |
| D | 0.0940 | 0.0363 | 0.0402 | 0.0107 |
| E | 0.4209 | 0.4130 | 0.2431 | 0.1433 |
| F | 0.7232 | 0.7261 | 0.6098 | 0.6062 |
| G | 0.0140 | 0.0138 | 0.0002 | 0.0000 |
| H | 0.1438 | 0.0414 | 0.0748 | 0.0149 |

如果后续要把结果写成预警图，F 应该作为重点目标；E 可以作为次重点；A、B、C、D、H 更适合谨慎描述为局部风险或低到中等风险信号。G 在当前基线情景下整体风险较低。
...
| target | geographic_position | original_drivers | spatial_lag | temporal_trend |
| --- | --- | --- | --- | --- |
| A | 0.3381 | 0.3011 | 0.2756 | 0.0852 |
| B | 0.4161 | 0.1586 | 0.3530 | 0.0722 |
| C | 0.1027 | 0.4393 | 0.3463 | 0.1117 |
| D | 0.1287 | 0.3711 | 0.4144 | 0.0859 |
| E | 0.1490 | 0.5716 | 0.2251 | 0.0544 |
| F | 0.1673 | 0.3085 | 0.4213 | 0.1029 |
| G | 0.2227 | 0.3874 | 0.3899 | 0.0000 |
| H | 0.1041 | 0.3156 | 0.5803 | 0.0000 |

A 和 B 的地理位置贡献最高，说明经纬度本身携带了较强的区域背景。C 和 E 更依赖原始驱动因子，说明已有环境变量对这两个目标更有解释力。D、F、G、H 的空间滞后贡献最高，说明训练期邻近污染水平对它们更关键。G 和 H 的时间趋势列为 0，是因为当前 SHAP 汇总中对应目标没有单独分出稳定的时间趋势贡献，不能据此理解为完全没有时间变化。
