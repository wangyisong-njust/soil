...
正式 `docs/` 目录只保留两份文档：本复现说明 `docs/复现.md` 和主报告 `docs/report.md`。各分析环节的权威产物是 `tables/`、`results/` 和 `figures/` 下的表格、结果与图件。历史分项说明已归档到 `archive/legacy_docs/` 和 `archive/dev_reports/`，不作为正式阅读入口。
